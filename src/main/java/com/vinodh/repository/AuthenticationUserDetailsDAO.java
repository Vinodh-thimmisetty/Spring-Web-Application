...
public interface AuthenticationUserDetailsDAO {
	...
	final String LIST_ALL_USERS = "SELECT * FROM VINODH.AUTHENTICATION_USER_DETAILS";
	...
	final String UPDATE_USER = "UPDATE VINODH.AUTHENTICATION_USER_DETAILS"
			+ " SET USER_ROLE = #{authenticationUser.role}, LAST_MODIFIED_BY = 	#{loggedInUser}"
			+ " WHERE USER_ID = #{authenticationUser.userId}";

	final String MERGE_USER = "MERGE INTO VINODH.AUTHENTICATION_USER_DETAILS SOURCE USING"
			+ " (SELECT 1 FROM DUAL) TARGET"
			/* Merge operation on Same Table, so using the DUAL */
			...
			/* Insert a New Record if User doesn't exists already */
	...
	final String DELETE_USER = "DELETE FROM VINODH.AUTHENTICATION_USER_DETAILS WHERE USER_ID = #{userId}";
